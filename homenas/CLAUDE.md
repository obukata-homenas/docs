...
2. **`operations.md`** — 構築完了後の運用ノウハウ (端末追加 / DNS バイパス対処 / コンテナ更新 / バックアップ確認 / 特定サイト疎通切り分け / トラブル対応)

`00-` 〜 `11-` は構築時のステップ手順。すでに構築済みなので、特定の章を見直す時だけ参照。
...
| バックアップ宛先 (HDD) | `/srv/dev-disk-by-uuid-3a9a7624-afbb-4993-a1dd-38e2bd1546f5/shares/backups/` (外付け移行は 2026-06-23 にドライブ SMART 故障で中止、内蔵のまま) |
...
- **Android のプライベート DNS と Chrome のセキュア DNS は AGH を素通りする**。Wi-Fi の DNS を AGH に向けても効かないことがあるので両方 OFF を確認 (`operations.md` §4)

## 編集する時の方針

- ドキュメントは Markdown。修正は最小差分で
- 手順を増やす時は既存の章立て (§0〜§11 / operations.md) のどこに属するかを先に決める
- 設定変更で本ディレクトリの記述と現状が乖離したら README 変更履歴に 1 行残す
