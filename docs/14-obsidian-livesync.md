...
# uid/gid が違う場合は `docker run --rm couchdb:3.4 id couchdb` で確認
...
Fauxton(管理UI): `http://<Tailscale IP>:5984/_utils`(NPM設定後は `https://obsidian.obukata.uk/_utils`)
...
3. エンドツーエンド暗号化パスフレーズを設定(推奨。Tailscale 経由とはいえ多重防御として)
