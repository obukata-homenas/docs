...
- **D2. 秘密情報**:`.env` は実秘密(DB パスワード / Cloudflare トークン等)を含む →
  **実 `.env` は gitignore、`.env.example`(値を伏せた雛形)だけ commit**。実秘密の在り処は docs に記載。【推奨】
  - 平文の秘密は private リポでも GitHub に上げない、を鉄則にする。
...
## Part A:docs を org へ(短い・先に片付ける)
1. GitHub で `obukata/home-nas` を org `obukata-homenas` へ Transfer(obukata の操作:repo Settings → Transfer ownership)。
2. `/opt/ops/docs` の remote を `git@github-private:obukata-homenas/home-nas.git` に張替え → `git fetch` で疎通確認。
3. (任意)AGENTS.md 等の参照文言を更新。
